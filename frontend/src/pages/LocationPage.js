...
// import { fetchLocations, addLocation, fetchSchedulesInRange, updateLocation } from "../api"; 
...
//                 .filter(s => s.careWorker && s.location) // Ensure careWorker and location exist
//                 .map(s => ({
...
//                 .filter(s => s.careWorker) // Ensure careWorker exists
...
//             } else {
//                 setApiError(err.response?.data?.message || "Failed to process location. Please check console.");
//             }
//         } finally {
//             setLoading(false); 
//             setTimeout(() => {
...
//                 <div className="flex-1 px-10 py-6 bg-white overflow-auto">
//                     <div className="flex justify-between items-center mb-6">
//                         <h1 className="text-3xl font-bold text-gray-900">Locations</h1>
//                         {isAuthorized && isAdmin && (
//                             <button
//                                 className="bg-indigo-600 text-white px-4 py-2 rounded-md flex items-center space-x-2 hover:bg-indigo-700 transition duration-200"
...
//                             <button
//                                 className="p-2 border border-blue-500 bg-blue-50 text-blue-600 rounded-md w-[120px] flex items-center justify-center hover:bg-blue-100 transition duration-200"
//                                 onClick={async () => {
...
//                     <div className="grid grid-cols-1 md:grid-cols-[2fr,3fr,1.5fr,1.5fr] gap-4 text-sm font-semibold text-gray-600 py-3 border-b border-gray-200">
...
//                             <div
//                                 key={loc._id}
//                                 className="grid grid-cols-1 md:grid-cols-[2fr,3fr,1.5fr,1.5fr] gap-4 py-3 items-center border-b border-gray-200 text-sm"
//                             >
//                                 <div className="text-gray-900 font-medium">{loc.name}</div>
//                                 <div className="text-[#0034AD] flex items-center">
//                                     <MapPinIcon className="h-4 w-4 mr-1 flex-shrink-0" />
//                                     <span className="">
//                                         {loc.address?.street}, {loc.address?.city}, {loc.address?.postalCode}, {loc.address?.country}
//                                     </span>
//                                 </div>
//                                 <div className="text-gray-700">{loc.workType}</div> 
...
//                 <div className="w-72 bg-gray-50 border-l border-gray-200 px-5 py-6 shadow-md flex-shrink-0">
...
//                     <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 transform transition-all duration-300 ease-out scale-95 opacity-0 animate-scale-in">
...
//                                     className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-base placeholder-gray-400"
...
//                                 <label className="block text-lg font-semibold text-gray-800 border-b pb-2 mb-3">
//                                     Location Address
//                                 </label>
//                                 <div>
...
//                                     className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition duration-200 text-lg font-medium"
//                                     disabled={loading}
//                                 >
//                                     Cancel
//                                 </button>
//                                 <button
//                                     type="submit"
//                                     className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition duration-200 text-lg font-medium shadow-md flex items-center justify-center"
//                                     disabled={loading}
//                                 >
//                                     {loading ? (
...
export default function LocationPage() {
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-100 font-sans">
            <div className="flex flex-1 flex-col md:flex-row"> {/* Changed to column on mobile, row on medium screens */}
                <div className="flex-1 px-4 py-4 md:px-10 md:py-6 bg-white overflow-auto"> {/* Adjusted padding */}
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6"> {/* Stacked on mobile */}
                        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 md:mb-0">Locations</h1> {/* Adjusted font size */}
                        {isAuthorized && isAdmin && (
                            <button
                    ...
                    {successMessage && (
                        <div
                            className="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-4 text-sm" // Adjusted font size
                            role="alert"
                        >
                            <p className="font-bold">Success!</p>
                            <p>{successMessage}</p>
                    ...
                    {apiError && (
                        <div
                            className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4 text-sm" // Adjusted font size
                            role="alert"
                        >
                            <p className="font-bold">Error!</p>
                            <p>{apiError}</p>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between mb-4 space-y-4 md:space-y-0"> {/* Stacked on mobile */}
                        ...
                        {isAdmin && (
                        <div className="flex items-center space-x-2 w-full md:w-auto"> {/* Full width on mobile */}
                            <button
                    ...
                    {/* Table Header - Hidden on small screens, shown as grid on medium and up */}
                    ...
                    {!loading &&
                        filteredLocations.map((loc) => (
                            <div
                                key={loc._id}
                                className="flex flex-col md:grid md:grid-cols-[2fr,3fr,1.5fr,1.5fr] gap-4 py-3 items-start md:items-center border-b border-gray-200 text-sm" // Changed to column on mobile
                            >
                                <div className="text-gray-900 font-medium">{loc.name}</div>
                                <div className="text-[#0034AD] flex items-center mt-2 md:mt-0"> {/* Adjusted margin for mobile */}
                                    <MapPinIcon className="h-4 w-4 mr-1 flex-shrink-0" />
                                    <span className="">
                                        {loc.address?.street}, {loc.address?.city}, {loc.address?.postalCode}, {loc.address?.country}
                                    </span>
                                </div>
                                <div className="text-gray-700 mt-2 md:mt-0">{loc.workType}</div> {/* Adjusted margin for mobile */}
                                <div className="flex justify-start md:justify-end w-full mt-3 md:mt-0"> {/* Full width on mobile, right aligned on medium */}
                                    {isAuthorized && isAdmin && (
                                        <button
                ...
                <div className="w-full md:w-72 bg-gray-50 border-t md:border-t-0 md:border-l border-gray-200 px-4 py-4 md:px-5 md:py-6 shadow-md flex-shrink-0"> {/* Full width on mobile, adjusted padding */}
                    <div className="bg-white rounded-lg shadow-sm p-4">
                        <h2 className="text-lg font-semibold text-gray-800 mb-3">Teams</h2>
                        <select
                            className="w-full border border-gray-300 px-3 py-2 rounded-md text-sm focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        <div className="mt-6 md:mt-10 text-sm text-gray-700"> {/* Adjusted margin */}
                            {scheduledWorkers.length > 0 ? (
                                <>
                                    <p className="font-semibold mb-2">
                                        Scheduled Workers at {selectedLocation || "All Locations"}:
            ...
            {(isModalOpen || isEditModalOpen) && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm md:max-w-md p-6 md:p-8 transform transition-all duration-300 ease-out scale-95 opacity-0 animate-scale-in"> {/* Adjusted max-width and padding */}
                        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 md:mb-6 text-center"> {/* Adjusted font size */}
                            {isEditModalOpen ? 'Edit Location' : 'Add New Location'}
                        </h2>
                        ...
                        <form onSubmit={handleFormSubmit} className="space-y-4 md:space-y-5"> {/* Adjusted spacing */}
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">
                                    Location Name
                                </label>
                                <input
                                    ...
                                    value={form.name}
                                    onChange={handleFormChange}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm placeholder-gray-400" // Adjusted padding and font size
                                    placeholder="e.g., Central Office"
                                    required
                                />
                            </div>

                            <div className="space-y-3 md:space-y-4 pt-2"> {/* Adjusted spacing */}
                                <label className="block text-base md:text-lg font-semibold text-gray-800 border-b pb-2 mb-3"> {/* Adjusted font size */}
                                    Location Address
                                </label>
                                <div>
                                    <label htmlFor="street" className="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4"> {/* Stacked on extra small screens, 2 cols on small and up */}
                                    <div>
                                        <label htmlFor="city" className="block text-sm font-medium text-gray-700 mb-1">
                                            City
                                        </label>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    value={form.workType}
                                    onChange={handleFormChange}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm bg-white" // Adjusted padding and font size
                                    required
                                >
                                    <option value="">Select a Work Type</option>
                                    <option value="Residential">Residential</option>
                            ...
                            <div className="flex justify-end space-x-2 pt-4 md:pt-6"> {/* Adjusted spacing and padding */}
                                <button
                                    type="button"
                                    onClick={handleCloseModal}
                                    className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition duration-200 text-base font-medium" // Adjusted padding and font size
                                    disabled={loading}
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition duration-200 text-base font-medium shadow-md flex items-center justify-center" // Adjusted padding and font size
                                    disabled={loading}
                                >
                                    {loading ? (
                                        <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
