...
const MePage = () => {
    ...
    useEffect(() => {
        const fetchAndFilterSchedules = async () => {
            ...
            try {
                ...
                console.log('Raw API response for all schedules:', response.data.data);
                ...
                const allFetchedSchedules = response.data.data || []; // Ensure it's an array
    ...
    // const userOrg= user.organization.name;
    // console.log(userOrg)
    if (!user) {
        navigate('/login');
    ...
    return (
        <div className="min-h-screen bg-gray-100 font-poppins flex flex-col md:flex-row"> {/* Changed to flex-col on mobile */}
            ...
            <div className="flex items-center justify-center px-4 py-2 rounded-lg bg-gray-400 text-black shadow-lg transform transition duration-300 ease-in-out mb-4">
                <span className="text-base mr-2">🏢</span> {/* Emoji */}
