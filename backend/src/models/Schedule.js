...
const scheduleSchema = new mongoose.Schema({
  organization: { // NEW FIELD: Schedule belongs to an organization
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Organization',
        required: true,
    },
  ...
  break: {
    type: Number, // Store break duration as a number (e.g., in minutes)   // Default to 0 minutes if not provided
    min: [0, 'Break duration cannot be negative'], // Validator for Number type
    // max: [60, 'Break duration cannot exceed 60 minutes'], // Example: Uncomment if you need a max
  },
  isPublished: {
